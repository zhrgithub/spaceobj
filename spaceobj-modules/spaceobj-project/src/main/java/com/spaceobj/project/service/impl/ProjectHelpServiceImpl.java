...
@Service
@RefreshScope
public class ProjectHelpServiceImpl extends ServiceImpl<ProjectHelpMapper, ProjectHelp> implements ProjectHelpService {
    ...
    public SaResult createProjectHelpLink(ProjectHelpBo projectHelpBo) {
        ...
        try {
            ...
            resultList = list.stream().filter(ph -> {
                return !ObjectUtils.isEmpty(ph) && ph.getCreateUserId().equals(sysUser.getUserId()) && ph.getPUUID().equals(projectHelpBo.getPUUID());
            }).collect(Collectors.toList());
            // 如果之前创建过那么直接返回之前创建过的
    ...
    public ProjectHelp getProjectHelpLink(String pUUID, String userId) {
        ...
        try {
            ...
            resultProjectHelp = projectHelpList.stream().filter(hp -> {
                return !ObjectUtils.isEmpty(hp) && userId.equals(hp.getCreateUserId()) && pUUID.equals(hp.getPUUID());
            }).collect(Collectors.toList());
