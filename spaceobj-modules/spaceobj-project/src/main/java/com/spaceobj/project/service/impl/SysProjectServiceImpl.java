...
public class SysProjectServiceImpl extends ServiceImpl<SysProjectMapper, SysProject> implements SysProjectService {
    ...
    public SaResult addProject(SysProject sysProject) {
        ...
        try {
            ...
            List<SysProject> resultSysProjectList = sysProjectList.stream().filter(p -> {
                return !ObjectUtils.isEmpty(p) && p.getContent().equals(sysProject.getContent());
    ...
    private List<SysProject> getSysProjectList() {
        ...
        try {
            ...
            if (hasKey) {
                list = redisService.getHashMapValues(RedisKey.PROJECT_LIST, SysProject.class).stream().filter(p -> !ObjectUtils.isEmpty(p) && !ObjectUtils.isEmpty(p.getCreateTime())).sorted(Comparator.comparing(SysProject::getCreateTime).reversed()).collect(Collectors.toList());
                return list;
            }
            ...
            if (hasKey) {
                list = redisService.getHashMapValues(RedisKey.PROJECT_LIST, SysProject.class).stream().filter(p -> !ObjectUtils.isEmpty(p)).sorted(Comparator.comparing(SysProject::getCreateTime)).collect(Collectors.toList());
                return list;
            }
            QueryWrapper<SysProject> queryWrapper = new QueryWrapper();
    ...
    public SysUser getSysUserByUserId(String userId) {
        ...
        try {
            boolean flag = redisService.hasKey(RedisKey.SYS_USER_LIST);
            if (flag) {
                List<SysUser> sysUserList = redisService.getHashMapValues(RedisKey.SYS_USER_LIST, SysUser.class);
