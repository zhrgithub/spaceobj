...
public class CustomerServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements CustomerUserService {
    ...
    public SaResult loginByEmail(LoginByEmailBo loginByEmailBo) {
        ...
        // 获取缓存中的验证码
        String emailCodeFromRedis = redisService.getCacheObject(loginByEmailBo.getEmail(), String.class);
     ...
     * @return
     */
    private SysUser registerUserObj(LoginOrRegisterBo loginOrRegisterBo, SysUser sysUser, String md5Password) {
    ...
    public SaResult sendMailCode(String account) {
        ...
        try {
            ...
            // 设置邮件的标题，内容，收件人
            ReceiveEmailBo receiveEmail = ReceiveEmailBo.builder().receiverEmail(sysUser.getEmail()).title("spaceObj").content("邮箱验证码:" + getEmailCodeVerify + "，有效期：3 分钟").build();
    ...
    public SaResult realName(SysUserBo user) {
        ...
        try {
            ...
            // 从缓存中获取当前登录用户的基本信息
            SysUser sysUser = redisService.getCacheMapValue(RedisKey.SYS_USER_LIST, user.getLoginId(), SysUser.class);
    ...
    public SaResult uploadFile(MultipartFile multipartFile) {
        ...
        String url = FileUtil.uploadFileTolocalServer(uploadConfigure.uploadPath, uploadConfigure.uploadUrl, fileName, multipartFile);
